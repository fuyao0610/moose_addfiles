...
std::vector<unsigned int>
PolycrystalICTools::assignPointsToVariables_solution(const std::vector<Point> & centerpoints,
                                            const Real op_num,
                                            const MooseMesh & mesh,
                                            const MooseVariable & var)
{
  ...
  // Assign grains to specific order parameters in a way that maximizes the distance, op_index = 0 refers to solution
  for (unsigned int grain = 0; grain < grain_num; grain++)
  {
    // Determine the distance to the closest center assigned to each order parameter
    if (grain >= op_num)
    {
      // We can set the array to the distances to the grains 0..op_num-1 (see assignment in the else
      // case)
      for (unsigned int i = 1; i < op_num; ++i)
      {
        min_op_dist[i] =
            mesh.minPeriodicDistance(var.number(), centerpoints[grain], centerpoints[i]);
        min_op_ind[assigned_op[i]] = i;
    ...
    // Assign the current center point to the order parameter that is furthest away.
    unsigned int mx_ind = 1;
    for (unsigned int i = 2; i < op_num; ++i) // Find index of max
      if (min_op_dist[mx_ind] < min_op_dist[i])
        mx_ind = i;
...
unsigned int
PolycrystalICTools::assignPointToGrain_multicircle(const Point & p,
                                       const std::vector<Point> & centerpoints,
                                       ...
                                       const Real maxsize)
{
  ...
  unsigned int min_index = grain_num;
  // Loops through all of the grain centers and finds the center that is closest to the point p
  for (unsigned int grain = 1; grain < grain_num; grain++)
  {
    min_distance = mesh.minPeriodicDistance(var.number(), centerpoints[grain], p);

    if (min_distance < radii[grain])
    {
      min_index = grain;
      break;
    }
