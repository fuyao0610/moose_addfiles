...
KKSMultiMaterialsAction::act()
{
  ...
  for (unsigned int j = 0; j < _op_num; j++) {
      v_etanames[j] = "eta" + Moose::stringify(j);
  }
  ...
  for (unsigned int j = 0; j < _op_num; j++)
  {
    ...
    //
    {
      ...
      std::string h_name = "h" + Moose::stringify(j);
      ...
      std::string kernel_name = "h" + Moose::stringify(j);
...

    {
      ...
      params.set<std::string>("f_name") = "F" + Moose::stringify(j);
      var_name_c[0] = "c" + Moose::stringify(j);
      params.set<std::vector<VariableName>>("args") = var_name_c;
      if (j == 0)  func_expression = "20*(c" + Moose::stringify(j) + "-" + Moose::stringify(_eq_c[0]) + ")^2";
      else func_expression = "20*(c" + Moose::stringify(j) + "-" + Moose::stringify(_eq_c[1]) + ")^2";
      params.set<std::string>("function") = func_expression;
      std::string kernel_name = "f" + Moose::stringify(j);
...

    {
      ...
      params.set<std::string>("f_name") = "Dh" + Moose::stringify(j);
      std::string kernel_name = "Dh" + Moose::stringify(j);
      _problem->addMaterial("DerivativeParsedMaterial", kernel_name, params);
    }
    ...
    //
    {
      ...
      params.set<std::string>("function_name") = "g" + Moose::stringify(j);
      std::string kernel_name = "g" + Moose::stringify(j);
