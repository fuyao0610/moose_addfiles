...
template <>
InputParameters
validParams<Polycrystal_solutionKernelAction>()
{
...
Polycrystal_solutionKernelAction::Polycrystal_solutionKernelAction(const InputParameters & params)
  : Action(params),
...
void
Polycrystal_solutionKernelAction::act()
{
  for (unsigned int op = 0; op < _op_num; ++op)
  {
    ...
    v.resize(_op_num - 1); //notice this is the _op_num - 1, so that the op_index itself isn't included
